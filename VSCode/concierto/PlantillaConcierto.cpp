...
void resolver(const vector<vector<int>>& beneficios, const vector<vector<bool>>& consentimientos, vector<int>& sol, vector<bool>& marcas, int n, int& vSol, int& mejorSol, int k){
        ...
        if(!marcas[i]){
            ...
            if(esValida(consentimientos, sol, k)){
                ...
                if(k == n - 1){
                    ...
                    marcas[i] = true;
                    resolver(beneficios, consentimientos, sol, marcas, n, vSol, mejorSol, k + 1);
                    marcas[i] = false;
                }
...
void resuelveCaso() {
    ...
    resolver(beneficios, consentimientos, sol, marcas, n, vSol, mejorSol, 0);
    // salida
