...
public:
    ...
    //en el peor caso el coste coste es n2, siendo n2 la longitud del conjunto set2
    bool operator<=(const Set<T>& set2) const{

        if(nelems > set2.nelems) return false;

        bool contiene = true;
        int i = 0, j = 0;
        while(i < nelems && contiene){

            if(array[i] < set2.array[j]) contiene = false;

            if(array[i] == set2.array[j]) i++;

            j++;
            if(j > set2.nelems) contiene = false;
        }
