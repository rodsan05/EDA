...
public:
   ...
   //funcion usada para sol
   //coste n, ya que es el coste de la funcion minimo
   T elemMin() const{
...
protected:
   ...
   //coste n, siendo n el numero de nodos
   void minimo(Link raiz, T& min) const{

      if(raiz == nullptr) return;

      if(raiz->elem < min) min = raiz->elem;
      minimo(raiz->left, min);
      minimo(raiz->right, min);
   }
